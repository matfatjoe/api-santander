...
namespace Matfatjoe\SantanderBoleto\Boleto\PDF;
...
class BoletoPDF
{
    ...
    {
        $this->dadosBoleto = $this->formatarDados($dadosBoleto);
        $options = [
        ...
        //apenas para corrigir uma permissao no MAC
        if (file_exists('/Applications/XAMPP/xamppfiles/htdocs/InforGerencial/temp')) {
            $this->dompdf->set_option('tempDir', '/Applications/XAMPP/xamppfiles/htdocs/InforGerencial/temp');
        }
    }
    ...
    {
        ob_start();
        $this->montarHTML($this->dadosBoleto);
    ...
    private function formatarDados($dadosBoleto)
    {
        ...
        if (isset($dadosBoleto->linhaDigitavel) && strlen(trim(($dadosBoleto->linhaDigitavel ?? ''))) == 47) {
            $dadosBoleto->linhaDigitavel = $this->formatarLinhaDigitavel($dadosBoleto->linhaDigitavel);
        }
